...
public class JCPJSSEUtil extends JCPSSLUtilBase {
	...
	//ru.CryptoPro.ssl.Provider.ALGORITHM
	public static final String ALGORITHM = "GostTLS";
	// isIbm = ru.CryptoPro.JCP.tools.Platform.isIbm
	...
	static {
	    ...
	    try {
	      ...
	      context = new JCPSSLContext(ALGORITHM); // Прямо указан алгоритм из cpSSL
	...
	public KeyManager[] getKeyManagers() throws Exception { // Сокращен
	    ...
	    KeyManagerFactory kmf = KeyManagerFactory.getInstance(KEYMANGER_ALG); // Прямо указан алгоритм из cpSSL
	...
	public TrustManager[] getTrustManagers() throws Exception { // Сокращен
		...
		TrustManagerFactory tmf = TrustManagerFactory.getInstance(KEYMANGER_ALG); // Прямо указан алгоритм из cpSSL
